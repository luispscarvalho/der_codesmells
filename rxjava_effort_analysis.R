#
# Development timeseries analisys for RXJAVA (https://github.com/ReactiveX/RxJava)
#
# By Luis Paulo, 2017-02-28
#
#loading csv
data <- read.csv('/misc/workspace/doutorado/temp/rxjava.csv', header=TRUE, sep=",")
...
combined_e <- matrix(c(dev_e, bug_e, dvb_e, chr_e), nrow = 109)
#creating timeseries 
timeseries_e <- ts(combined_e, frequency = 1, names = c('enh', 'bug', 'e+b', 'churn'))
#plotting timeseries
plot(timeseries_e, main = "RxJava's Dev x Churn")
...
ndata <- data[15:54, 4:9] # high correlation interval
...
legend("topright", churn_range[2], c("God Class","Brain Method","Long Method"), cex=0.8, 
